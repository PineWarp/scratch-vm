...
runtimeFunctions.repeatString = `const repeatString = (string, count) => {
    count = Math.floor(count);
    return count < 0 || !Number.isFinite(count) ? '' : string.repeat(count);
}`;

runtimeFunctions.replaceString = `const replaceString = (substring, string, replacement) => string.replace(
